...
import { generateFixSuggestion } from '../metadata/fixSuggestions/fixSuggester.js';
import { getCachedFix, setCachedFix } from '../metadata/fixSuggestions/fixCache.js';
import { getApiKey } from '../js/settings.js';
...
function getFindingId(f) {
  return btoa(`${f.file || ''}:${f.line || ''}:${f.pattern}:${f.analyzer}`)
    .replace(/[=+/]/g, '')
    .slice(0, 20);
}
...
async function handleAnalyze() {
  ...
  try {
    ...
    const statsText = `Done. Scanned ${result.fileCount} files, analyzed ${result.analyzedCount}, found ${result.findings.length} findings.`;
    const tipText =
      'Not every scanned file is analyzed. Files are filtered based on relevance to the detected Scale Center signals. Wrong metadata type, test classes, managed packages, or files outside the analyzer scope are excluded.';
    progress.innerHTML = '';
    const statsSpan = document.createElement('span');
    statsSpan.className = 'scan-stats';
    statsSpan.textContent = statsText + ' ';
    const infoSpan = document.createElement('span');
    infoSpan.className = 'scan-stats-info';
    infoSpan.title = tipText;
    infoSpan.setAttribute('aria-label', tipText);
    infoSpan.innerHTML =
      '<svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="16" x2="12" y2="12"/><line x1="12" y1="8" x2="12.01" y2="8"/></svg>';
    statsSpan.appendChild(infoSpan);
    progress.appendChild(statsSpan);
    lastAnalysisResult = result;
    findingsById = new Map();
    result.findings.forEach((f) => {
      f._id = getFindingId(f);
      findingsById.set(f._id, f);
    });
...
function renderGroupedFindings(findings, container) {
  ...
  container.querySelectorAll('.ai-fix-btn').forEach((btn) => {
    btn.addEventListener('click', () => handleFixClick(btn));
  });
...
function renderFindingCard(f) {
  ...
  const fixId = f._id || getFindingId(f);
  ...
  return `
    <div class="finding-card severity-${f.severity}">
      ...
      <div class="finding-ai-fix">
        <button class="ai-fix-btn" data-finding-id="${fixId}" aria-label="Get AI fix suggestion for this finding">
          <span class="ai-fix-icon">&#10024;</span>
          <span class="ai-fix-label">Get AI fix suggestion</span>
          <span class="ai-fix-cost">~$0.002</span>
        </button>
        <div class="ai-fix-result ai-fix-result--hidden" id="fix-result-${fixId}"></div>
      </div>
    </div>`;
}
...
async function handleFixClick(btn) {
  const findingId = btn.dataset.findingId;
  const finding = findingsById.get(findingId);
  if (!finding) return;

  const resultEl = document.getElementById(`fix-result-${findingId}`);
  const model = localStorage.getItem('orgpulse-ai-fix-model') || 'haiku';

  // Check cache
  const cached = getCachedFix(finding, model);
  if (cached) {
    renderFixResult(resultEl, cached, true);
    return;
  }
  ...
  if (!apiKey) {
    resultEl.classList.remove('ai-fix-result--hidden');
    resultEl.innerHTML =
      '<div class="ai-fix-error">Claude API key not configured. Open Settings (gear icon) to add your Anthropic API key.</div>';
    return;
  }

  // Loading state
  btn.disabled = true;
  btn.querySelector('.ai-fix-label').textContent = 'Generating...';
  resultEl.classList.remove('ai-fix-result--hidden');
  resultEl.innerHTML = '<div class="ai-fix-loading">Analyzing with Claude...</div>';

  try {
    const fix = await generateFixSuggestion({ finding, apiKey, model });
    setCachedFix(finding, model, fix);
    renderFixResult(resultEl, fix, false);
  } catch (err) {
    resultEl.innerHTML = `<div class="ai-fix-error">${err.message}${err.retriable ? ' <button class="ai-fix-retry" onclick="this.closest(\'.finding-ai-fix\').querySelector(\'.ai-fix-btn\').click()">Try again</button>' : ''}</div>`;
  } finally {
    btn.disabled = false;
    btn.querySelector('.ai-fix-label').textContent = 'Get AI fix suggestion';
  }
}

function renderFixResult(el, fix, fromCache) {
  const modelLabel = fix.model === 'sonnet' ? 'Sonnet' : 'Haiku';
  const cacheNote = fromCache ? ' (cached)' : '';
  el.classList.remove('ai-fix-result--hidden');
  el.innerHTML = `
    <div class="ai-fix-meta">Generated by Claude ${modelLabel}${cacheNote}</div>
    <div class="ai-fix-body">${renderSimpleMarkdown(fix.text)}</div>
    ${fix.model !== 'sonnet' ? '<div class="ai-fix-regen"><button class="ai-fix-regen-btn">Regenerate with Sonnet (deeper analysis)</button></div>' : ''}
  `;
  // Wire regenerate button
  const regenBtn = el.querySelector('.ai-fix-regen-btn');
  if (regenBtn) {
    regenBtn.addEventListener('click', async () => {
      const parentFix = el.closest('.finding-ai-fix');
      const mainBtn = parentFix.querySelector('.ai-fix-btn');
      const findingId = mainBtn.dataset.findingId;
      const finding = findingsById.get(findingId);
      if (!finding) return;

      const cachedSonnet = getCachedFix(finding, 'sonnet');
      if (cachedSonnet) {
        renderFixResult(el, cachedSonnet, true);
        return;
      }
      ...
      regenBtn.textContent = 'Generating with Sonnet...';

      try {
        const sonnetFix = await generateFixSuggestion({
          finding,
          apiKey,
          model: 'sonnet',
        });
        setCachedFix(finding, 'sonnet', sonnetFix);
        renderFixResult(el, sonnetFix, false);
      } catch (err) {
        el.innerHTML += `<div class="ai-fix-error">${err.message}</div>`;
      }
    });
  }
}
...
function renderSimpleMarkdown(md) {
  ...
  // Code blocks first
  html = html.replace(/```(\w*)\n([\s\S]*?)```/g, (_, _lang, code) => {
    return `<pre class="ai-fix-code"><code>${code.replace(/</g, '&lt;').replace(/>/g, '&gt;')}</code></pre>`;
  });
  html = html.replace(/`([^`]+)`/g, '<code class="ai-fix-inline-code">$1</code>');
  html = html.replace(/^## (.+)$/gm, '<h4 class="ai-fix-heading">$1</h4>');
  html = html.replace(/^### (.+)$/gm, '<h5>$1</h5>');
  html = html.replace(/\*\*([^*]+)\*\*/g, '<strong>$1</strong>');
  html = html
    .split(/\n\n+/)
    .map((block) => {
      if (block.match(/^<(pre|h[1-6]|ul|ol)/)) return block;
