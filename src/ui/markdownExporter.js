...
import { getCachedFix } from '../metadata/fixSuggestions/fixCache.js';
...
function buildFindingBlock(f) {
  ...
  // AI fix suggestion if cached
  const cachedFix = getCachedFix(f, 'sonnet') || getCachedFix(f, 'haiku');
  if (cachedFix) {
    lines.push('');
    lines.push(
      `**AI Fix Suggestion** *(generated by Claude ${cachedFix.model === 'sonnet' ? 'Sonnet' : 'Haiku'})*`
    );
    lines.push('');
    lines.push(cachedFix.text);
    lines.push('');
  }
